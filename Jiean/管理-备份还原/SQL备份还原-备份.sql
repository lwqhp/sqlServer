...
,NAME ='备份集的名称'
,MEDIADESCRIPTION ='媒体集的自由格式文本，帮助识别媒体的内容'
,MEDIANAME ='整个备份媒体集的名称，最多为128个字符'

--b)定义写入备份集方式
,RETAINDAYS=30 --指定在这个备份集上保留时间，超过这个时间才会被覆盖
,INIT |NOINIT --是覆盖还是追加到当前备份集(表示备份时需要覆盖备份媒体，并在备份媒体上将该 备份作为第一个文件写入)
...
--d)备份设定
,DIFFERENTIAL --差异备份
,compression | NO_COMPRESSION --是否压缩备份
