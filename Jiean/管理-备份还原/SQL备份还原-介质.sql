...
/*可以简单的这样理解：就是下面框框里的备份文件，像备份设备，可以覆盖，追加，失效等。
单个或镜像备份划分成簇，一般的有几个备份文件就分成几个簇,这只是一种逻辑划分标识
备份介质则是实际的备份载体.(备份介质是实体，备份集是逻辑体，介质集是备份集集合实体，介质簇是逻辑体)
...
当你得到一个备份时，有时候可能会不知道备份集中包含的备份信息
 ...
 从标签，头信息，文件信息来查看备份集的信息,
*/

--RESTORE 语句查询备份文件备份（媒体）集信息
RESTORE LABELONLY FROM DISK='F:\DBBak\ad01.bak' --查看备份集的标签信息
...
--查看备份集所有信息
RESTORE HEADERONLY FROM DISK='F:\DBBak\ad01.bak' --查看备份集的头部信息
