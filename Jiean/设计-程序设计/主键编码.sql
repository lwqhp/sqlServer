...
--主键编码
/*
主键通常分自然主键和代理键，几个字段组合称为组合主键。

自然主键 是指实际数据中存在唯一标识的字段，而使用来作为表格主键。
代理键 是指额外增加的和数据语义没有关联的，仅用来标识数据唯一性的数字序列。

这里主要重点讲下代理键 的设计方法和优缺点
...
identity键有些限制，比如依赖于表，不能随意生成新值。
标识值在insert语句执行时生成，不是在执行之前生成，对于先生成标识值，还插入的需求不能实现
以异步方式分配标识值，有可能出现断号。
*/
