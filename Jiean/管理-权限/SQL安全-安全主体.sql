...
--安全主体
/*
一个能请求服务器，数据库或架构资源的实体称为安全主体.

主体是操作的对象,登陆时的window帐套，sql帐户，和数据库里的用户,就是我们常说的主体
SqlServer把安全级别分成三个级别
window级别
sqlServer级别
	数据库级别

不同的安全级别决定了主体的影响范围，通常，window和sqlserver级别的安全主体具有实例级的范围，而数据库级别的主体其
影响范围是特定的数据库。服务器级别主体和数据库主体是相互独立的(是指验证和授权)，又是关联的(每一个服务器主体都对
应一个数据库主体，数据库对象是针对数据库主体授权)
...
--查看实例中已经添加的window登录名和用户组(里面包含了window主体和sql服务器主体，以及固定角色)
select * from sys.server_principals
where type_desc in('WINDOWS_LOGIN','WINDOWS_GROUP')
/*
几个内部window主体：
NT SERVICE\SQLWriter: SQL Server VSS 编写器,SQLWriter 服务在具有所需的所有权限的 LOCAL SYSTEM 帐户下运行
NT SERVICE\Winmgmt :  Windows Management Instrumentation (WMI) 必须能够连接到数据库引擎。 为了支持这一点，应在数据库引擎中提供 Windows WMI 提供程序 (NT SERVICE\winmgmt) 的 Per-service SID。 
					SQL Server 安装程序将 NT SERVICE\Winmgmt 帐户设置为数据库引擎登录名，并将其添加到 sysadmin 固定服务器角色中
NT Service\MSSQLSERVER : 数据库引擎服务的默认实例
NT AUTHORITY\SYSTEM :  Local System 是一个具有高特权的内置帐户。 它对本地系统拥有许多特权并作为网络上的计算机。 该帐户的实际名称为 NT AUTHORITY\SYSTEM。 
NT SERVICE\SQLSERVERAGENT : SQL Server 代理服务，位于以下默认实例上：SQL Server
NT SERVICE\ReportServer : sqlserver报表服务
