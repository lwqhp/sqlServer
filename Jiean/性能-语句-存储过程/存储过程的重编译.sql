...
5）SET选项变化,在存储过程中修改set选项会导致sqlServer在执行set语句后面的语句之前重编译该存储过程，因此，在执行
...
/*触发第一次执行发生重编译,存储过程创建生成的执行计划不包
含任何关于局部临时表的信息，因此，生成的计划不会用于使用DML语句访问临时表
重编译后，再次运行 insert into语句
*/

SELECT * FROM #myTemptable 
/*触发第2次重编译，来自于该表装入时其中包含的数据的变化
重编译后，再次运行语句
*/


exec('SELECT * FROM #myTemptable')
/*excel会引起1,5的语句重编译，但不会造成存储过程的重编译,因为exec语句改变了临时表架构、绑定或权限*/

EXECUTE sp_executesql N'SELECT * FROM #myTemptable' 
/*sp_executesql不会引起存储过程和语句的重编译*/


--alter table #myTemptable add newfiled varchar(30)
/*第一次执行发生延迟对象解析编译，第二次以后每次执行发生架构改变重编译*/

CREATE CLUSTERED INDEX PK_myTemptable ON #myTemptable(id)

SELECT * FROM #myTemptable
/*触发第3次重编译，是由于临时表的架构变化使现在计划作废，导致在表再次被访问时进行重编译
...
SELECT * FROM #t2
/*第4次重编译，生成一个包含#t2的处理策略的计划*/

go

exec sptemp2 
--主要是第一次执行会出现延迟对象解析,第二次执行则会重用计划缓存,修改了存储过程，不管有没有变，再次执行都会发生重编译
