...
/*
对于排名计算，其最佳索引是在分区列，排序列，覆盖列(作为包含列，而不是键列)

为了计算排名值，优化器需要先按分区列再按排序列对数据进行排序

排名函数中的order by 子句是必需的，如果只是生成序号，而不按照任何特定的顺序来进行排名计算，这样可以避免由于
按顺序扫描索引或排序数据而带来的额外开销，可以这样写：
row_number() over(order by (select 0)) 或over(order by @v)

在某种程度上，row_number函数之所以这么快是因为它不须要在任何地方返回结果，基于游标和基于 identity的解决方案
须要要把结果保存到一个表中，以备后用，因此，这两种方法会产生大量的io,当然，如果你须要实例化地保存结果集，用
identity会快些，因为它只扫描一次表。
