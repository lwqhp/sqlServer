...
以实例名开头:Buffer manager:提供了计数器，用于监视sqlserver如何使用内存存储数据页，内部数据结构和过程缓存。
	1，Buffer Cache Hit Ritio:在缓冲区高速缓存中找到而不需要从磁盘中读取的页的百分比。
...
更改数据库设计等方法，将page reads降低。
	...
	9，Stolen pages : 用于非database pages(包括执行计划缓存)的页数，这里就是stole memory在buffer pool里的大小
...
意，并不一定意味着buffer pool里的stolen 内存和multi-page 内存没有问题)
	...
	11，total pages:缓冲池中的页数(包括数据库页，可用页和stolen页) 乘以8kb,就应该是total server memory 的值。
