...
规律：对于任意一个级别的汇总行（级别：group by 级别1，级别2，级别3）的汇总行，总是:
		该级别(级别2)的分组弄上使用grouping(级别2)=1,
		该级别(级别1)的分组弄上使用grouping(级别1)=0,	
所以：当要过滤或者处理这些级别是上的汇总记录行时，
使用grouping(当前级别的列)=1 and grouping(当前级别的前一列)=0来定位行。		
*/
